...
// We need to set up the worker in a useEffect to ensure it only runs on client
...
export function PdfViewer({ pdfId, pdfUrl, initialPage, userId }: PdfViewerProps) {
  ...
  // Show loading while worker is being set up
  if (!workerReady) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
    ...
    >
      ...
      <div className="sticky top-0 z-10 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b px-2 sm:px-4 py-2 sm:py-3">
        <div className="flex items-center justify-between max-w-4xl mx-auto gap-1 sm:gap-2">
          ...
          {/* Zoom controls - hidden on mobile, show percentage only */}
          <div className="flex items-center gap-1 sm:gap-2">
            {!isMobile && (
              <>
                <Button
